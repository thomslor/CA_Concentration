...
def insert_db(domain,rank,subject,country, CA1, CA1_ORG, CA1_C, ICA, ICA_ORG, ICA_C, RCA, RCA_ORG, RCA_C):
    c.execute('''INSERT INTO CA_100K_6(URL, RANK, SUBJECT, COUNTRY, CA1, CA1_ORG, CA1_C, ICA, ICA_ORG, ICA_C, RCA, RCA_ORG, RCA_C) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
              (str(domain),
               rank,
               str(subject),
               str(country),
...
def current_db(cursor):
    list_db = []
    cursor.execute("SELECT URL FROM CA_100K_6")
...
if __name__ == "__main__":
    ...
    conn.execute('''CREATE TABLE if not exists CA_100K_6
                (URL TEXT NOT NULL,
                RANK INT,
                SUBJECT TEXT,
                COUNTRY TEXT,
